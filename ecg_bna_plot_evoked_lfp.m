...
for hs = 1:size(evoked_lfp, 2)
    if ~isempty([evoked_lfp(:,hs).mean]) &&  ~all(all(isnan([evoked_lfp(:,hs).mean]))) && ~isempty([evoked_lfp(:,hs).std])
        ...
        if isfield(evoked_lfp(1, hs), 'colors') && ~isempty(evoked_lfp(1, hs).colors)
            colors = evoked_lfp(1, hs).colors;
        end
        for i = 1:size(evoked_lfp(1, hs).mean, 1)
            plot(evoked_lfp(1, hs).time, evoked_lfp(1, hs).mean(i, :), 'Color', colors(i,:));
        end
        ...
        if isfield(evoked_lfp(1, hs), 'shuffled_mean') && isfield(evoked_lfp(1, hs), 'shuffled_std')
            for i = 1:size(evoked_lfp(1, hs).shuffled_mean, 1)
                plot(evoked_lfp(1, hs).time, evoked_lfp(1, hs).shuffled_mean(i, :), 'Color', colors(i),'linestyle','-.');
                plot(evoked_lfp(1, hs).time, evoked_lfp(1, hs).shuffled_mean(i, :) + evoked_lfp(1, hs).shuffled_std(i, :), [colors(mod(i, length(colors))) ':']);
                plot(evoked_lfp(1, hs).time, evoked_lfp(1, hs).shuffled_mean(i, :) - evoked_lfp(1, hs).shuffled_std(i, :), [colors(mod(i, length(colors))) ':']);
            end
        end
