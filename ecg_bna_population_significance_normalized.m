function [significance, pmap]=ecg_bna_population_significance_normalized(norm_data)
...
for s = 1:numPermutation % loop through shuffles
    ...
    for k=1:size(real_P,2)
        for m=1:size(real_P,3)
            if ~all(isnan(realRtmp(:,k,m)))
                [P(k,m),H(k,m),STATS(k,m)] = signrank(realRtmp(:,k,m));
            else
                [P(k,m),H(k,m),STATS(k,m)] = deal (NaN);
            end
        end
    end
    if isfield(STATS,'zval')
        Z=reshape([STATS.zval],size(STATS));
        % Find significant pixels
    else
        Z=reshape([STATS.signedrank],size(STATS));
        % Find significant pixels
    end
    pos_thresh = (P < pthreshold) & Z>0 ;
    neg_thresh = (P < pthreshold) & Z<0 ;
    ...
    % max sum of the tmap
    sumTpos = cellfun(@(x) sum(Z(x)),pos_clusters.PixelIdxList);
    sumTneg = cellfun(@(x) sum(Z(x)),neg_clusters.PixelIdxList);
...
for k=1:size(real_P,2)
    for m=1:size(real_P,3)
        if ~all(isnan(real_P(:,k,m)))
            [P(k,m),H(k,m),STATS(k,m)] = signrank(real_P(:,k,m));
        else
            [P(k,m),H(k,m),STATS(k,m)] = deal (NaN);
        end
    end
