...
site_ecg = rmfield(sites,{'LFP','MUA','ECG'}); %% remove ecg too

fprintf('=============================================================\n');
fprintf('Processing site, %s\n', sites.site_ID);
site_ecg.session = sites.site_ID(1:12);
site_ecg.recorded_hemisphere = upper(sites.target(end));
...
ts=round(cfg.ecg.timestep/ts_original);
...
site_ecg.tfs.resampling_factor=1/ts;
site_ecg.tfs.sr=1/ts_original/ts;
if dospectralanalysis
    sizepreallocator=[size(frequency_bands,1),floor(numel(sites.LFP)/ts)];
    site_ecg.tfs.phabp=NaN(sizepreallocator);
    site_ecg.tfs.powbp=NaN(sizepreallocator);
    sizepreallocator=[numel(frequencies),floor(numel(sites.LFP)/ts)];
    site_ecg.tfs.pha=NaN(sizepreallocator);
    site_ecg.tfs.pow=NaN(sizepreallocator);
    %site_ecg.tfs.lfp=NaN([1,floor(numel(sites.LFP)/ts)]);
end

site_ecg.tfs.ecg=NaN([1,floor(numel(sites.ECG)/ts)]);
...
for b=1:numel(blocks_with_LFP) 
    ...
    be=floor(sum(sites.ECG_samples(b_samples))/ts)+samples_past_resampled;  
    bs_original=samples_past+1;
    be_original=sum(sites.ECG_samples(b_samples))+samples_past;  
    samples_past=be_original;
    samples_past_resampled=be;    
    
    concat_raw = real(double(sites.ECG(bs_original:be_original))); % scale here is really bad 
    %% for some freaking reason, ECG can be a complex......
    
    site_ecg.tfs.n_samples_per_block(:,b)=[B,be-bs+1];
    dat = nanmean(reshape(concat_raw(1:end-mod(size(concat_raw,2), ts)),ts,[]),1);
    site_ecg.tfs.ecg(1,bs:be)= dat;
    ...
    if dospectralanalysis
        ...
        for f=1:length(frequencies)
            ...
            site_ecg.tfs.pha(f,bs:be)= dat./abs(dat);
            % extracted Power of each trial
            site_ecg.tfs.pow(f,bs:be) = abs(dat).^2;
        end
        ...
        for f=1:size(frequency_bands,1)
            ...
            absH=abs(H);
            site_ecg.tfs.phabp(f,bs:be) = H./absH;
            site_ecg.tfs.powbp(f,bs:be) = absH.^2;
        end
    end
end
if dospectralanalysis
site_ecg.tfs.pha=site_ecg.tfs.pha(:,1:be);
site_ecg.tfs.pow=site_ecg.tfs.pow(:,1:be);
site_ecg.tfs.phabp=site_ecg.tfs.phabp(:,1:be);
site_ecg.tfs.powbp=site_ecg.tfs.powbp(:,1:be);
end

site_ecg.tfs.ecg=site_ecg.tfs.ecg(:,1:be);
end
