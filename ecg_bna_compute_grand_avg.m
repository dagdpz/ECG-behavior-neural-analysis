    ...
    for f = 1:length(all_lfp_data)
        ...
        for c = 1:length(triggered_site_data.condition)
            ...
            for e=1:size(cfg.analyse_states,1)
                ...
                evokedR       = squeeze(event.observed.evoked.mean)';
                itpcR      = squeeze(event.observed.itpc.mean);
                powerR     = squeeze(event.observed.pow.mean);
                itpcbpR    = squeeze(event.observed.itpcbp.mean);
                powerbpR   = squeeze(event.observed.powbp.mean);
                
                
                evokedS       = squeeze(event.surrogate.evoked.mean)';
                itpcS      = squeeze(event.surrogate.itpc.mean);
                powerS     = squeeze(event.surrogate.pow.mean);
                itpcbpS    = squeeze(event.surrogate.itpcbp.mean);
                powerbpS   = squeeze(event.surrogate.powbp.mean);
                ...
                sites(s).condition(c).event(e).evoked_R = evokedR;
                ...
                sites(s).condition(c).event(e).evoked_S = evokedS;
...
fileName = fullfile([cfg.fldr.LFP_root filesep cfg.monkey,'_',cfg.analyse_states{1, 2} ,'_Triggered_target_wise_Grand_grand_avg_sessions_targets',withunits,'.mat']);
...
if reprocess2
    for t = 1: length(targets)
        ...
        for c = 1:size(condition,2)
            ...
            for e=1:size(cfg.analyse_states,1)
                to_cat={'evoked','pow','itpc','powbp','itpcbp',};
...
if plotit
    ...
    for t = 1: length(targets)
        ...
        for c = 1:length(cond)
            ...
            results_file{c} = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},'-',tar(t).con(c).cond_name,'-','avg of ',num2str(tar(t).nSites),' sites ',withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        for t = 1: length(targets)
            for c = 1:length(cond)
                ...
                results_file{c,t} = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},'-',tar(t).con(c).cond_name,'-',E,'-','avg of ',num2str(tar(t).nSites),' sites ',withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        results_file = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',E,'-power,' withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        results_file = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',E,'-itpc,' withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        for t = 1: length(targets)
            for c = 1:length(cond)
                ...
                results_file{c,t} = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},'-',tar(t).con(c).cond_name,'-',E,'-','sig% of ',num2str(tar(t).nSites),' sites ',withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        results_file = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',E,'-','sig% of evoked, ',withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        for t = 1: length(targets)
            for c = 1:length(cond)
                ...
                results_file{c,t} = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},ncond,E,'-','signed sig%',nsites,withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        for t = 1:length(targets)
            ...
            results_file = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},'-',E,'-Max_ITPCbp_POWbp_of ',num2str(tar(t).nSites),' sites ', withunits]);
    ...
    for e=1:size(cfg.analyse_states,1)
        ...
        for t = 1:length(targets)
            ...
            mtit([strrep(tar(t).target,'_','-') '-' E],mtitsettings{:});
            results_file = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},'-',E,'-Time_of_Max_ITPCbp_POWbp ',num2str(tar(t).nSites),' sites ', withunits]);
            export_fig(h,[results_file,'.pdf']);
        end
        close all,
        clc
    ...
    %     results_file = fullfile(cfg.fldr.LFP_root, [cfg.monkey,'-',targets{t},'-','Time_of_Max_ITPCbp_POWbp',nsites,withunits]);
    %     export_fig(h,[results_file,'.pdf']);
    % end
    % clearall
    %
