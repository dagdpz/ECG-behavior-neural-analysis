...
for b=1:numel(out)
	...
	allowed_jitter_range=min(max(out(b).R2R_valid)-min(out(b).R2R_valid),min(out(b).R2R_valid));%std(out(b).R2R_valid)*6;                    % one-sided std of normally distributed jitter values
