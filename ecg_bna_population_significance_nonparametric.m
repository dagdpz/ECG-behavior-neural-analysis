function [significance, pmap]=ecg_bna_population_significance_nonparametric(real,surr)
...
for s = 1:numPermutation % loop through shuffles
    ...
    for k=1:size(real_P,2)
        for m=1:size(real_P,3)
            [P(k,m),H(k,m),STATS(k,m)] = signrank(realRtmp(:,k,m),surrRtmp(:,k,m));
        end
    end
...
for k=1:size(real_P,2)
    for m=1:size(real_P,3)
        [P(k,m),H(k,m),STATS(k,m)] = signrank(real_P(:,k,m),surr_P(:,k,m));
    end
end
Z=reshape([STATS.zval],size(STATS));
% Find significant pixels
