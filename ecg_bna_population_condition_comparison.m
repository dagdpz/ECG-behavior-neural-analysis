...
for t = 1: length(targets)
    for e=1:size(cfg.analyse_states,1)
        ...
        itpc_data_rest = tar(t).con(1).(eventname).itpc; % grand adverage of MD target, Rest Condition , across grand_avg(2).nSites;
        itpc_data_task = tar(t).con(2).(eventname).itpc; % grand adverage of MD target, Task Condition ,across grand_avg(2).nSites;
        tfr_freq = freq;
        tfr_time = tar(t).con(1).(eventname).tfr_time;
        
        
        rest_R = permute(itpc_data_rest, [3, 2, 1]);  % [trials x  time x frequencies]
        task_R = permute(itpc_data_task, [3, 2, 1]);  % [trials x  time x frequencies]
        
        
        % generating the shuffled files :
        tic
        task_shf=zeros([numPermutation,size(task_R,2),size(task_R,3)]);
        rest_shf=zeros([numPermutation,size(task_R,2),size(task_R,3)]);
        ...
        T=squeeze(sum(shf<repmat((task_R-rest_R),[size(shf,1),1,1]),1));
        T=T/(size(shf,1)-1)-0.5;
        ...
        subplot(131)
        imagesc(tfr_time, 1:numel(tfr_freq),(mean(itpc_data_task,3)));
        set(gca,'YDir','normal');
        ...
        title(['pow - ',cond{2},' - nSite: ', num2str(size(itpc_data_task,3))],'fontsize', 8,'Interpreter', 'none');
        % ==============================================================
        % Plotting the Rest data
        subplot(132)
        imagesc(tfr_time, 1:numel(tfr_freq),(mean(itpc_data_rest,3)));
        set(gca,'YDir','normal');
        %     set(gca, 'YScale', 'log');  % If you want to show the y-axis in logarithmic scale
        fbandstart = unique(frequency_bands(:))';
        set(gca,'Xlim',[-.25 .25]);
        ...
        colormap(jet);  % Change the colormap if desired
        colorbar;  % Show colorbar
        title(['pow - ',cond{1},' - nSite: ', num2str(size(itpc_data_rest,3))],'fontsize', 8,'Interpreter', 'none');
        
        % ==============================================================
        % Plotting the Stats data
        subplot(133)
        imagesc(tfr_time, 1:numel(tfr_freq),(mean(itpc_data_task,3)- mean(itpc_data_rest,3)));
        ...
        colormap(jet);  % Change the colormap if desired
        colorbar;  % Show colorbar
        title(['pow - ',cond{2},' vs. ',cond{1},' difference - Cluster t-Values' ],'fontsize', 8,'Interpreter', 'none');
        ...
        data_diff = (mean(itpc_data_task,3)- mean(itpc_data_rest,3));
        set(gca,'clim', [min(min(data_diff)) max(max(data_diff))])
        
        sgtitle([monkey,'-',targets{t},'-','pow -',cond{2},'-',cond{1},'- Significant difference - numPerm =',...
            num2str(cfg_perm.numrandomization)],'fontsize', 12,'Interpreter', 'none');
        results_file = fullfile([path_to_save,filesep,monkey,'-',targets{t},'-','pow_Significant difference']);
