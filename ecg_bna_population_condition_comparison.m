...
switch data_type
    case 'lfp'
        if strcmp(monkey,'Bacchus')
            if filtered
                path_to_save = ['Y:\Projects\Pulv_bodysignal\LFP\ECG_Bacchus_TaskRest_generalTrig_finalRun\4hz filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\'];
                load(['Y:\Projects\Pulv_bodysignal\LFP\ECG_Bacchus_TaskRest_generalTrig_finalRun\4hz filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\Bacchus_Rpeak_Triggered_target_wise_Grand_grand_avg_sessions_sitesall.mat'])
            else
                path_to_save = ['Y:\Projects\Pulv_bodysignal\LFP\ECG_Bacchus_TaskRest_generalTrig_finalRun\not filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\'];
                load(['Y:\Projects\Pulv_bodysignal\LFP\ECG_Bacchus_TaskRest_generalTrig_finalRun\not filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\Bacchus_Rpeak_Triggered_target_wise_Grand_grand_avg_sessions_sitesall.mat'])
            end
        elseif strcmp(monkey,'Magnus')
            if filtered
                path_to_save = ['Y:\Projects\Pulv_bodysignal\LFP\ECG_Magnus_TaskRest_generalTrig_finalRun\4hz filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\'];
                load(['Y:\Projects\Pulv_bodysignal\LFP\ECG_Magnus_TaskRest_generalTrig_finalRun\4hz filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\Magnus_Rpeak_Triggered_target_wise_Grand_grand_avg_sessions_sitesall.mat'])
            else
                path_to_save = ['Y:\Projects\Pulv_bodysignal\LFP\ECG_Magnus_TaskRest_generalTrig_finalRun\not filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\'];
                load(['Y:\Projects\Pulv_bodysignal\LFP\ECG_Magnus_TaskRest_generalTrig_finalRun\not filtered\grand_average_w combine_hemispheres_LFP ',normversion,'_New GrandAvg version\Magnus_Rpeak_Triggered_target_wise_Grand_grand_avg_sessions_sitesall.mat'])
            end
        end
        ...
        for t = 1: length(targets)
            for fin = 1:length(Fin)
                for e=1:size(cfg.analyse_states,1)
                    ...
                    if strcmp(Fin{fin},'lfp') && strcmp(monkey,'Magnus')
                        y1 = [-4 4];
...
%                     mua       = squeeze(event.real.mua.mean)'; % not normalized MUA
        ...
        for t = 1: length(targets)
            for fin = 1:length(Fin)
                for e=1:size(cfg.analyse_states,1)
                    ...
                    data_rest = squeeze(tar(t).con(1).(eventname).(Fin{fin})); % grand adverage of MD target, Rest Condition , across grand_avg(2).nSites;
                    data_task = squeeze(tar(t).con(2).(eventname).(Fin{fin})); % grand adverage of MD target, Task Condition ,across grand_avg(2).nSites;
                    mua_time = tar(t).con(1).(eventname).mua_time;
                    ...
                    shadedErrorBar(mua_time,smoothed_meanT,smoothed_stdT,linePropsT,1); hold on;
                    shadedErrorBar(mua_time,smoothed_meanR,smoothed_stdR,linePropsR,1);
...
%                     y1 = ylim;
                    for i = 1:CCpos.NumObjects
                        idx = CCpos.PixelIdxList{i};
                        t_start = mua_time(idx(1));
                        t_end = mua_time(idx(end));
                    ...
                    ylabel('MUA evoked Potential (A.U.)');
                    results_file = fullfile([path_to_save,filesep,monkey,'-',targets{t},'-',Fin{fin},'-',cond{2},' vs. ',cond{1},'_Significant difference']);
