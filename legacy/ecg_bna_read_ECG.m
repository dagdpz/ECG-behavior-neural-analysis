function session_ecg = ecg_bna_read_ECG( session_info, plottrials )
...
for i_Files = 1:length(combined_Blocks)
    ...
    for b = 1:length(by_block_ECG)
        ...
        for t = 1:length(trial)
            ...
            session_ecg.trials(t).completed = trial(t).completed;
            session_ecg.trials(t).type = trial(t).type;
            session_ecg.trials(t).effector = trial(t).effector;
            session_ecg.trials(t).run = trial(t).run;
            session_ecg.trials(t).block = block;
            session_ecg.trials(t).dataset = [];
            session_ecg.trials(t).choice_trial = trial(t).choice;
            session_ecg.trials(t).success = trial(t).success;
            session_ecg.trials(t).reach_hand = 0;
            session_ecg.trials(t).reach_space = 0;
            session_ecg.trials(t).fix_pos = trial(t).fix_pos;
            session_ecg.trials(t).eye_pos = trial(t).tar_pos; + trial(t).sac_off; %??
            ...
            for st = 1:length(trial(t).states)
                ...
                state_onset = trial(t).states_onset(st);
                % get sample number of state onset time
                state_onset_sample = find(abs(timestamps - state_onset) == min(abs(timestamps - state_onset)), 1);
                % save into struct
                session_ecg.trials(t).states(st).id = state_id;
                session_ecg.trials(t).states(st).onset_t  = state_onset;
                session_ecg.trials(t).states(st).onset_s  = state_onset_sample;
            end
        end
