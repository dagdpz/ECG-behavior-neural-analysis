...
for t = 1:length(trials_lfp)
    ...
    for w = 1:length(window_mid_idx)
        ...
        ecg_triggered_evoked.lfp = [ecg_triggered_evoked.lfp, lfp_data(window_mid_idx(w) - round(nsamples_window/2):window_mid_idx(w) + round(nsamples_window/2))];
        % timestamps
        ecg_triggered_evoked.lfp_time =  lfp_time(window_mid_idx(w) - round(nsamples_window/2):window_mid_idx(w) + round(nsamples_window/2)) ;
        % set mid-timestamp to zero
        ecg_triggered_evoked.lfp_time = ecg_triggered_evoked.lfp_time - ecg_triggered_evoked.lfp_time(round(length(ecg_triggered_evoked.lfp_time)/2));
        ecg_triggered_evoked.time     = [ecg_triggered_evoked.time, ecg_triggered_evoked.lfp_time];
    end       
end
