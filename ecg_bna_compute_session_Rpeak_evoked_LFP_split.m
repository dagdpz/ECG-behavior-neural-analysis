function [ session_evoked ] = ecg_bna_compute_session_Rpeak_evoked_LFP_split( session_proc_lfp, session_ecg,analyse_states, ecg_bna_cfg )
    ...
    for cn = 1:length(site_conditions)
        ...
        for hs = 1:length(hs_labels)
            ...
            for st = 1:size(analyse_states, 1)
                cond_trials_lfp = session_proc_lfp(i).trials(cond_trials(cix));
                [cond_trials_lfp.ECG_spikes] = session_ecg.trials(trial_idx).ECG_spikes;
                if strcmp(analyse_states{st, 1}, 'ecg')
                    state_evoked = ecg_bna_get_Rpeak_evoked_LFP_fast(cond_trials_lfp, analyse_states(st, :));
                    shuffled_Rpeak_evoked.mean=zeros(size(state_evoked.mean));
                    shuffled_Rpeak_evoked.lfp =zeros(size(state_evoked.mean));
                    ...
                    if isfield(ecg_bna_cfg, 'random_permute_triggers') && ecg_bna_cfg.random_permute_triggers
                        ...
                        [cond_trials_lfp.ECG_spikes]=session_ecg.trials(trial_idx).ECG_spikes_shuffled;
                        shuffled_evoked = ecg_bna_get_Rpeak_evoked_LFP_fast(cond_trials_lfp, analyse_states(st, :));
                ...
                if ~isempty(state_evoked.lfp)
                    ...
                    sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).trials = cond_trials(cix); %find(cond_trials);
        ...
        if condition_valid && ~isempty(fieldnames(sites_evoked(i).condition(cn).hs_tuned_evoked)) % &&~isempty([sites_evoked(i).condition(cn).hs_tuned_evoked.lfp])
            ...
            result_file = fullfile(site_results_folder, ['LFP_Evoked_' sites_evoked(i).site_ID '_' site_conditions(cn).label]);
    ...
    for dcn = 1:length(sites_evoked(i).difference)
        if ~isempty(sites_evoked(i).difference(dcn).hs_tuned_evoked)
            if isfield(sites_evoked(i).difference(dcn).hs_tuned_evoked,'mean')
                ...
                result_file = fullfile(site_results_folder, ['LFP_DiffEvoked_' sites_evoked(i).site_ID '_' 'diff_condition' num2str(dcn)]);
    ...
    for cn = 1:length(site_conditions)
        ...
        for st = 1:size(analyse_states, 1)
            for hs = 1:length(hs_labels)
                ...
                session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp = [];
            end
        end
        for i = 1:length(sites_evoked)
            ...
            if sites_evoked(i).use_for_avg
                ...
                if isempty(sites_evoked(i).condition(cn).hs_tuned_evoked) || ~isfield(sites_evoked(i).condition(cn).hs_tuned_evoked, 'mean')
                    continue;
                end
                for hs = 1:size(sites_evoked(i).condition(cn).hs_tuned_evoked, 2)
                    for st = 1:size(sites_evoked(i).condition(cn).hs_tuned_evoked, 1)
                        ...
                        session_avg(t).condition(cn).hs_tuned_evoked(st, hs).nsites = session_avg(t).condition(cn).hs_tuned_evoked(st, hs).nsites + 1;
                        if session_avg(t).condition(cn).hs_tuned_evoked(st, hs).nsites == 1
                            % subtracting shuffled mean
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp = [session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp;...
                                sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).shuffled_mean] ;
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).lfp = [session_avg(t).condition(cn).hs_tuned_evoked(st, hs).lfp;...
                                sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).mean - sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).shuffled_mean] ;
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).time = sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).time;
                            ...
                            % subtracting shuffled mean
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp  = [session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp(:,1:nsamples); ...
                                sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).shuffled_mean(1:nsamples)] ;
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).lfp  = [session_avg(t).condition(cn).hs_tuned_evoked(st, hs).lfp(:,1:nsamples); ...
                                sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).mean(1:nsamples)- sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).shuffled_mean(1:nsamples)] ;
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).time = session_avg(t).condition(cn).hs_tuned_evoked(st, hs).time(1:nsamples) ;
                            
                        end
                        % struct to store average evoked LFP across sites
                        session_avg(t).condition(cn).hs_tuned_evoked(st, hs).hs_label = sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).hs_label;
                        if isfield(sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs), 'state') && isfield(sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs), 'state_name')
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).state = sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).state;
                            session_avg(t).condition(cn).hs_tuned_evoked(st, hs).state_name = sites_evoked(i).condition(cn).hs_tuned_evoked(st, hs).state_name;
                        end
                    end
        ...
        for hs = 1:size(session_avg(t).condition(cn).hs_tuned_evoked, 2)
            for st = 1:size(session_avg(t).condition(cn).hs_tuned_evoked, 1)
                session_avg(t).condition(cn).hs_tuned_evoked(st, hs).mean = nanmean(session_avg(t).condition(cn).hs_tuned_evoked(st, hs).lfp, 1);
                session_avg(t).condition(cn).hs_tuned_evoked(st, hs).std = nanstd(session_avg(t).condition(cn).hs_tuned_evoked(st, hs).lfp, 0, 1);
                session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_mean = nanmean(session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp, 1);
                session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_mean = nanstd(session_avg(t).condition(cn).hs_tuned_evoked(st, hs).shuffled_lfp, 0, 1);
            end
        end
        ...
        ecg_bna_plot_evoked_lfp (session_avg(t).condition(cn).hs_tuned_evoked, ecg_bna_cfg, plottitle, result_file);
    end
