function ecg_triggered_evoked = ecg_bna_get_state_evoked_ECG( trials, label, cfg_ecg, signal_type,state )
...
for t = 1:length(trials)
    ...
    s=[trials(t).states.id]==state;
    if ~any(s)
        continue;
    end
    start_time=trials(t).tstart;
    ts=trials(t).tsample;
    nsamples = numel(trials(t).ecg_data);
    end_time = start_time + (ts*(nsamples-1));
    timestamps = linspace(start_time, end_time, nsamples);
    state_onset=trials(t).states(s).onset_t;
    
    onset = find(abs(timestamps - state_onset(1)) == min(abs(timestamps - state_onset(1))), 1);
    
    %onset=[trials(t).states(s).onset_s];
    
    %signal_time = trials(t).time;
    state_onsets = false(size(timestamps));
    state_onsets(onset)=true;
    ft_data_all.trial = [ft_data_all.trial, [signal; state_onsets]];
    ft_data_all.time = [ft_data_all.time, timestamps];
